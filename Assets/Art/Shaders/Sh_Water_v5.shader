...
////// Emissive:
                ...
                float3 emissive = (step(0.5,((lerp(sin(((_WaterRingSpeed_var*node_70.g)+(_WaterRingAmount_var*ShallowDepthBlend)+(_WaterRingWobblyness_var*node_5060.r))),ShallowDepthBlend,_WaterRingDepthInfluence_var)*node_3968)+pow(node_3968,5.0)))+lerp(lerp(_VeryDeepColor_var.rgb,_DeepColor_var.rgb,DeepDepthBlend),_ShallowColor_var.rgb,DepthBlendFloat));
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float _MidDepthBlend_var = UNITY_ACCESS_INSTANCED_PROP( Props, _MidDepthBlend );
                float DepthBlendFloat = (1.0 - saturate(saturate((sceneZ-partZ)/_MidDepthBlend_var)));
                o.Emission = (step(0.5,((lerp(sin(((_WaterRingSpeed_var*node_70.g)+(_WaterRingAmount_var*ShallowDepthBlend)+(_WaterRingWobblyness_var*node_5060.r))),ShallowDepthBlend,_WaterRingDepthInfluence_var)*node_3968)+pow(node_3968,5.0)))+lerp(lerp(_VeryDeepColor_var.rgb,_DeepColor_var.rgb,DeepDepthBlend),_ShallowColor_var.rgb,DepthBlendFloat));
